...
export default function HistoryVisits() {
  const styleCards = {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <Box className="container" key={el.id} style={styleCards}>
          ...
          >
            ...
            <Grid item xs={6}>
              ...
              >
                Дата обработки:
                {` ${el.drug_date.toLocaleDateString()}`}
              </Typography>
            </Grid>
          </Grid>
        </Box>
      ...
      {pet.Visits.map((el) => (
        <Box className="container" key={el.id} style={styleCards}>
          <Typography variant="h6" component="div" sx={{ fontWeight: 'bold', m: '1rem' }}>
            Врач:
            {` ${el.doc_id.first_name} ${el.doc_id.last_name}`}
          </Typography>

          <Box sx={{ display: 'flex', justifyContent: 'space-between', mx: '1rem' }}>
            <Typography
              variant="p"
              component="span"
              sx={{ fontWeight: 'bold' }}
            >
              Дата визита:
              {` ${el.visit_date.toLocaleDateString()}`}
            </Typography>
